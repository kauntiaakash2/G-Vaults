...
export default function CaseDetailPage() {
  ...
  const canUpload = Boolean(me.data && ['ADMIN', 'INVESTIGATOR', 'SENIOR_OFFICER', 'DEPARTMENT_HEAD'].includes(me.data.role));
  ...
  return <>
    ...
    {tab === 'documents' && <section className="section-surface" id="case-documents" role="tabpanel"><div className="section-heading"><div><p className="section-kicker">Document register</p><h2>Case documents</h2><p>Protected source files and their current versions.</p></div>{canUpload && <button className="button secondary" aria-expanded={showUpload} aria-controls="upload-panel" onClick={() => setShowUpload((value) => !value)}><Icon name={showUpload ? 'close' : 'plus'}/>{showUpload ? 'Close upload' : 'Upload document'}</button>}</div>
      {showUpload && <form className="form-grid inset-form" id="upload-panel" onSubmit={submit}><label htmlFor="documentTitle"><span>Document title</span><input id="documentTitle" name="title" minLength={2} maxLength={200} required/></label><label htmlFor="documentType"><span>Document type</span><input id="documentType" name="documentType" placeholder="REPORT" minLength={2} maxLength={50} required/><small>Use a concise organisational type.</small></label><label className="full file-field" htmlFor="documentFile"><span>Source file</span><input id="documentFile" name="file" type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.docx,.txt,.md" required/><small>PDF, image, DOCX, text or Markdown. Maximum size follows server policy.</small></label>{upload.error && <div className="full"><StateMessage tone="danger" title="Upload failed" detail={`${upload.error.message} The source case remains unchanged.`}/></div>}<div className="form-actions full"><button className="button secondary" type="button" onClick={() => setShowUpload(false)}>Cancel</button><button className="button primary" type="submit" disabled={upload.isPending}>{upload.isPending ? 'Protecting and uploading…' : 'Upload document'}</button></div></form>}
      {documents.isPending ? <SkeletonRows rows={4}/> : documents.error ? <StateMessage tone="danger" title="Documents could not be loaded" detail={documents.error.message}/> : documents.data?.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Document</th><th>Type</th><th>Version</th><th>File</th><th>Storage</th><th>Updated</th><th><span className="sr-only">Open</span></th></tr></thead><tbody>{documents.data.map((document) => <tr key={document.id}><td><Link className="primary-cell" href={`/documents/${document.id}`}><strong>{document.title}</strong><span>{document.createdBy.name}</span></Link></td><td>{document.documentType}</td><td>v{document.currentVersion?.versionNumber ?? '—'}</td><td><span className="cell-stack"><strong>{document.currentVersion?.originalFilename ?? 'No file'}</strong><small>{formatBytes(document.currentVersion?.fileSize ?? 0)}</small></span></td><td><StatusBadge tone="success">Protected</StatusBadge></td><td>{formatRelative(document.updatedAt)}</td><td><Link className="row-action" aria-label={`Inspect ${document.title}`} href={`/documents/${document.id}`}><Icon name="chevron"/></Link></td></tr>)}</tbody></table></div> : <StateMessage title="No documents have been added to this case" detail={canUpload ? 'Upload the first authorised document to begin the case record.' : 'You do not currently have permission to upload documents.'} action={canUpload && <button className="button primary" onClick={() => setShowUpload(true)}><Icon name="plus"/>Upload document</button>}/>} 
    </section>}
